...
export function TeamDashboard({ team, initialAssignments }: Props) {
  ...
  const [activeTab, setActiveTab] = useState<"learning" | "challenges">("challenges");
  ...
  const fetchLearningMaterials = useCallback(async (module?: "ai" | "cybersecurity" | "all") => {
    try {
      setLoadingMaterials(true);
      const moduleToUse = module || selectedModule;
      const moduleParam = moduleToUse === "all" ? "" : `?module=${moduleToUse}`;
      const data = await fetch(`/api/learning${moduleParam}`).then((res) => res.json());
  ...
  // Fetch materials when module changes
  useEffect(() => {
    if (activeTab === "learning") {
      fetchLearningMaterials();
    }
  }, [selectedModule, activeTab, fetchLearningMaterials]);
            ...
            <section className="rounded-3xl bg-white p-6 shadow">
              <div className="mb-4 flex items-center justify-between">
                <div>
                  ...
                  <h2 className="mt-1 text-2xl font-semibold">Learning Materials</h2>
                </div>
